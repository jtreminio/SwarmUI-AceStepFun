...
The `run-tests` script lives in **this extension’s root directory**: the folder that contains `SwarmUI-AceStepFun.csproj`, `SwarmUI-AceStepFun.Tests.sln`, and `run-tests`. It is **not** at the main SwarmUI repository root and not inside `src/`, `Tests/`, or `frontend/` unless that is already the extension root.
...
2. **Call it by path from the SwarmUI repo root**:
   ...
   `src/Extensions/SwarmUI-AceStepFun/run-tests`
